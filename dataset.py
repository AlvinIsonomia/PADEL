...
class GraphLoader:
    def __init__(self, hyper_params):
        ...
        # List of diffused mask (np.array, 1 for original node, 0 for diffused neighbors)
        self.diffused_mask = []
    ...
    def __load_from_file(self, input_hyper_params):
        ...
        if input_hyper_params['diffuse']:
            ...
            self.diffused_mask = get_diffused_mask(input_hyper_params['data_name'], 
                                                   input_hyper_params['subgraph_file'])
        else:
            temp_subgraph_path = os.path.join(input_hyper_params['data_path'],
                                              input_hyper_params['subgraph_file'])
        print("Using ", temp_subgraph_path, 'as input subgrpaphs')
        with open(temp_subgraph_path, 'rt') as f:
            for line in tqdm(f.readlines()):
                ...
                self.sub_nodes[mode].append((nodes, label_idx))
